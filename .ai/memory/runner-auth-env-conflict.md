...
On 2026-07-30, dispatching a task to the `claude` runner (`.ai/runners.yaml`)
...
Chosen fix: the `claude` entry in `.ai/runners.yaml` prefixes the command
...
entry in `.ai/runners.yaml` must preserve this env-var stripping (or an
