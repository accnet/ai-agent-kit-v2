...
On 2026-07-30, dispatching a task to the `claude` runner (`.ai/runners.json`)
...
Chosen fix: the `"claude"` entry in `.ai/runners.json` prefixes the command
...
```
"claude": "env -u ANTHROPIC_AUTH_TOKEN -u ANTHROPIC_BASE_URL claude -p {prompt} --permission-mode acceptEdits"
```

Consequence / do not revert: if this `env -u ...` prefix is removed or the
`claude` runner command is rewritten from scratch, dispatch to `claude` will
...
entry in `.ai/runners.json` must preserve this env-var stripping (or an
